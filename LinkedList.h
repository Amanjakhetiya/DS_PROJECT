...
void createStudent(char *sName, char *numberOfStudent, float marks[]) {
    ...
    struct Student *n = (struct Student*) malloc(sizeof(struct Student)); // Allocates memory to new Node
    strcpy(n->Name, sName);     // Copy Students Info into New Node
    strcpy(n->numberOfStudent, numberOfStudent);
    int j;  for (j = 0; j < max; j++) {
        n->Marks[j] = marks[j];}
    n->next = NULL;
    addingStudent(n); // call add function to add student
}
